...
public class AdminGearController {
 ...
 @GetMapping public String list(@RequestParam(required = false) String q, Model m){String query=q == null ? "" : q.trim();m.addAttribute("query",query);m.addAttribute("gears",query.isEmpty() ? gears.findAllByOrderByIdAsc() : gears.search(query));return "admin/gears";}
